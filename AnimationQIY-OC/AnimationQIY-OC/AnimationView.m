...
//    [self addSubview:centerImage];
    ...
    self.icon = icon;
    icon.frame=CGRectMake(0,0, self.bounds.size.width*0.7, self.bounds.size.width*0.7);
    icon.center=CGPointMake(self.bounds.size.width/2, self.bounds.size.width/2);
    icon.image = [UIImage imageNamed:@"96x96"];
    ...
    //设置路径的颜色
    maskLayer.strokeColor=[UIColor colorWithRed:76.0/255 green:164.0/255 blue:254.0/255 alpha:1.00f].CGColor;
    //设置路径的宽度
...
-(void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag
{
    ...
    if([[anim valueForKey:@"animationName"] isEqualToString:@"BasicAnimationEnd"])
    {
        ...
        [self.icon.layer addAnimation:BasicAnimation forKey:@"BasicAnimationRotation"];
